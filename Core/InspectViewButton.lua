...
hooksecurefunc(WCollections.AceAddon, "ADDON_LOADED", function(self, event, addon)
    if addon == "Blizzard_InspectUI" then
        if not button and not InspectFrameViewButton then
            button = CreateFrame("Button", "InspectFrameViewButton", InspectPaperDollFrame, "WCollectionsUIPanelButtonTemplate");
            button:SetSize(0, 22);
    ...
    elseif addon == "Examiner" then
        if not button and not ExaminerViewButton then
            button = CreateFrame("Button", "ExaminerViewButton", Examiner.model, "WCollectionsUIPanelButtonTemplate");
            button:SetSize(0, 22);
