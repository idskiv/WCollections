...
function WCollections:JoinWintergrasp()
    self:SendAddonCommand(".menu wg queue");
end
...
local function ElvUIHook()
    ...
    S:HandleButton_WCollections(button);
...
WCollections:MergeHook("WCollectionsElvUIHook", function()
    local E = unpack(ElvUI);
    local S = E:GetModule("Skins");
    S:AddCallback("Skin_PvP_WCollections", ElvUIHook);
end);

function WCollections:SetWintergraspButton(enabled)
    if enabled then
        if not button then
            button = CreateFrame("Button", nil, PVPBattlegroundFrame, "UIMenuButtonStretchTemplate");
            button:SetPoint("RIGHT", PVPBattlegroundFrame, "TOPRIGHT", -40+2, -55);
            button:HookScript("OnClick", function(self)
                PlaySound("gsTitleOptionOK");
                WCollections:JoinWintergrasp();
            end);
