...
local function addapi(object)
	...
	function mt.StripTextures_WCollections(object, kill, alpha)
		object:StripTextures();
		if not object:IsObjectType("Texture") then
			local FrameName = object.GetName and object:GetName()
			for _, Blizzard in pairs(StripTexturesBlizzFrames) do
				local BlizzFrame = object[Blizzard] or (FrameName and _G[FrameName..Blizzard])
				if BlizzFrame and BlizzFrame.StripTextures and BlizzFrame.StripTextures_WCollections then
					BlizzFrame:StripTextures_WCollections(kill, alpha)
				end
			end
		end
	end
...
function S:HandleButton_WCollections(button, ...)
...
function S:HandleInsetFrame_WCollections(frame)
...
function S:HandlePortraitFrame_WCollections(frame, createBackdrop)
	assert(frame, 'doesnt exist!')
	...
	frame:StripTextures_WCollections()
	...
	if insetFrame then
		S:HandleInsetFrame_WCollections(insetFrame)
	end
...
do
	...
	function S:HandleMaxMinFrame_WCollections(frame)
...
if MF then
	function MF:ModelWithControls_WCollections(model)
