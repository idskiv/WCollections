...
E:RegisterCallback("Skin_DressingRoom", function()
	...
	S:HandlePortraitFrame_WCollections(DressUpFrame, true)
	S:HandleMaxMinFrame_WCollections(DressUpFrame.MaximizeMinimizeFrame)
	...
	SetToggleIcon(DressUpFrame.ToggleOutfitDetailsButton, [[Interface\AddOns\WCollections\Interface\Icons\70_Professions_Scroll_01]])
	...
	WardrobeOutfitFrame:StripTextures_WCollections(true)
	...
	SideDressUpFrame:StripTextures_WCollections();
...
if MF then
	...
	function MF:ModelWithControls(model)
		if model == DressUpModel then
			MF:ModelWithControls_WCollections(model);
		else
			old(self, model);
		end
	end
	...
	hooksecurefunc(MF, "Initialize", function()
		if E.private.enhanced.character.modelFrames then
			MF:ModelWithControls_WCollections(SideDressUpModel);
		end
	end);
end
