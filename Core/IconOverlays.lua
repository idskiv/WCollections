local L = WCollections.L;
WCollections.IconOverlays = WCollections.IconOverlays or { };
local function Config() return WCollections.Config.IconOverlays; end
...
local overlayIconInfo =
{
    ...
    {
        Custom = true,
        Texture = [[Interface\AddOns\WCollections\Interface\ContainerFrame\Bags]],
    ...
    {
        Custom = true,
        Texture = [[Interface\AddOns\WCollections\Interface\ContainerFrame\CosmeticIconBorder]],
...
local function MakeOptions(key, list, formatter)
    ...
    local k2i = { };
    local i2k = { };
    for key, value in WCollections:Ordered(list, function(a, b) return a.Order < b.Order; end) do
...
local textureOptions = MakeOptions("Texture", overlayTextures, function(key, value) return format([[|TInterface\AddOns\WCollections\Textures\IconOverlays\%s:22:22:0:-1|t]], unpack(value)); end);
...
function WCollections.IconOverlays:MakeOptions()
    local config = Config();
    local expandedType = { };
        ...
        args =
        {
                            ...
                            args[type] =
                            {
                                ...
                                args =
                                {
                                    Enable =
                                    {
                                        ...
                                        dialogControl = "WCollectionsOptionsCheckBoxWithSettingsTemplate",
                                        arg =
                                        {
                                    ...
                                    Settings =
                                    {
                                        ...
                                        args =
                                        {
                                            ...
                                            CustomColor =
                                            {
                                                ...
                                                width = "half",
                                                order = 400,
                                                handler = WCollections.ConfigHandlers.CustomColor, get = "get", set = "set",
                                                arg = config,
                                            },
            ...
            Addons =
            {
                ...
                args = (function()
                    ...
                    for addon, module in WCollections:Ordered(WCollections.IconOverlays:GetAddons(), function(_, _, a, b) return a == "WCollections" or b ~= "WCollections" and a < b; end) do
                        local blizzard = addon == "WCollections";
                        args[addon] =
                        {
                            type = "toggle",
                            name = blizzard and L["Config.Integration.ItemButtons.Addons.Blizzard"] or WCollections.ConfigHelpers.IntegrationAddonName(addon),
                            width = "full",
                            order = order,
                            ...
                            dialogControl = module.GetOptions and "WCollectionsOptionsCheckBoxWithSettingsTemplate" or nil,
    ...
    for _, type in ipairs(overlayTypes) do
        if config[type].Enable then
            if type == "Cosmetic" then
                if WCollections.hasCosmeticItems then
                    return true;
                end
...
local function MakeOverlayIconInfo(type)
    ...
    info.Texture = [[Interface\AddOns\WCollections\Textures\IconOverlays\]]..(unpack(texture) or "")..(unpack(style) or "") or nil;
...
local function GetOverlayType(item)
    item = item and WCollections.GetItemID(item);
    if not item or item == WCollections.MenuItemBack then return; end
    local config = Config();
    if config.Known.Enable or config.Unknown.Enable then
        local status = WCollections:GetCollectibleStatus(item);
        if status ~= nil then
    ...
    if Config().ShowRecipes then
        local productType = GetOverlayType(WCollections:GetDressableFromRecipe(item));
        if productType then
            return productType;
        end
    end

    if config.Unowned.Enable and WCollections:HasOwnedItem(item) == false then
        return "Unowned";
    end
end
...
local function UpdateOverlay(button, name, type, condition, overrideLayout)
    ...
    local overlay = button.WCollectionsOverlay and button.WCollectionsOverlay[name];
    if info and info.Enable and condition then
        if not overlay then
            ...
            if not button.WCollectionsOverlay then
                button.WCollectionsOverlay = CreateFrame("Frame", nil, parent or button);
            end
            overlay = button.WCollectionsOverlay:CreateTexture(nil, overlayLayers[name] or "OVERLAY");
            button.WCollectionsOverlay[name] = overlay;
        end
        ...
        local frame = button.WCollectionsOverlay;
...
local function checkItem(bagNumber, slotNumber, itemButton)
    ...
    if item then
        ...
        if config.Cosmetic.Enable and WCollections.hasCosmeticItems and WCollections:IsSkinSource(item) then
            local _, _, _, flags = WCollections:GetItemTransmog("player", bagNumber, slotNumber);
...
local deferredBags = { };
local BAG_ALL = -2;
function WCollections.IconOverlays:Update(bag, force, deferredCall)
        ...
        if not deferred then
            ...
            C_Timer.After(0, function()
                ...
                if deferredBags[BAG_ALL] then
                    WCollections.IconOverlays:Update(nil, true, true);
                else
                    for bag in pairs(deferredBags) do
                        WCollections.IconOverlays:Update(bag, true, true);
                    end
                end
...
function WCollections.IconOverlays:GetAddons() return addons; end
...
local function Event(...)
    ...
    for i = 1, numParams - 1 do
        ...
        if not WCollections.AceAddon[event] then
            WCollections.AceAddon[event] = hook;
            WCollections.AceAddon:RegisterEvent(event);
        else
            hooksecurefunc(WCollections.AceAddon, event, hook);
        end
    end
end
...
-- Trade Skill
-- Guild Bank
hooksecurefunc(WCollections.AceAddon, "ADDON_LOADED", function(self, event, addon)
    ...
    ["WCollections"] =
    {
        ...
        Hook = function()
            BankFrame:HookScript("OnShow", function()
                addons.WCollections.blizzardBagsCreated = true;
            end);
            hooksecurefunc("ContainerFrame_GenerateFrame", function(frame, size, id)
                addons.WCollections.blizzardBagsCreated = true;
                WCollections.IconOverlays:Update(id);
            end);
        end,
        Bags = function(bag)
            if not addons.WCollections.blizzardBagsCreated then return; end
