...
--[[
    ...
    WCollections.Mounts[id][3] = flags (Mount.db2):
    0x001 - Server Only
    0x002 - Is Self Mount
...
local function IsMountUsable(mountID)
    local _, type = WCollections:GetMountInfo(mountID);
    ...
    if bit.band(type or 0, 0x1) ~= 0 and not IsFlyableArea() then
        if WCollections:IsMountScalingAllowed() then
            withScaling = true;
        else
            return false;
        end
...
local function MatchesFilter(mountID)
    ...
    local _, type, flags = WCollections:GetMountInfo(mountID);

    -- Completely hide certain mounts from the list
    if shouldHideOnChar and not WCollections.Config.Wardrobe.MountsShowHidden then
        return false;
    end

    if WCollections:IsActiveMountSubscriptionMount(mountID) then
    ...
    if not _showUnusable then
        ...
        if WCollections.Config.Wardrobe.MountsUnusableInZone then
...
local function SelectRandomFavoriteMount()
    ...
    for companionIndex = 1, GetNumCompanions("MOUNT") do
        ...
        if isUsable then
            ...
            if WCollections:GetMountFavoritesContainer()[mountID] then
...
local function CreateFavoriteMacro(perChar)
    local macro = CreateMacro(WCollections.L["Macro.Mount"], FindMacroIcon([[Interface\Icons\Ability_Mount_RidingHorse]]) or 1, _macroBody, perChar);
    MacroFrame_Update();
    return macro;
end
...
local function ShowMacroPopup()
    LoadAddOn("Blizzard_MacroUI");
    StaticPopupDialogs["WCOLLECTIONS_MOUNT_MACRO_CREATE"].OnAccept = function(self)
        C_Timer.After(0, function()
            if InCombatLockdown() then
                StaticPopup_Show("WCOLLECTIONS_ERROR", WCollections.L["Popup.Error.Mount.Macro.Combat"]);
                return;
            end
            ...
            elseif numChar < MAX_CHARACTER_MACROS then
                StaticPopup_Show("WCOLLECTIONS_MOUNT_MACRO_PERCHARACTER");
            else
                StaticPopup_Show("WCOLLECTIONS_ERROR", WCollections.L["Popup.Error.Mount.Macro.NoSpace"]);
            end
        end);
    end;
    StaticPopupDialogs["WCOLLECTIONS_MOUNT_MACRO_PERCHARACTER"].OnAccept = function(self)
        C_Timer.After(0, function()
            if InCombatLockdown() then
                StaticPopup_Show("WCOLLECTIONS_ERROR", WCollections.L["Popup.Error.Mount.Macro.Combat"]);
                return;
    ...
    StaticPopup_Show("WCOLLECTIONS_MOUNT_MACRO_CREATE");
end
...
function C_MountJournal.RefreshMounts() -- Custom
    ...
    for mountID, info in pairs(WCollections.Mounts) do
    ...
    for companionIndex = 1, GetNumCompanions("MOUNT") do
        ...
        if oldMounts and not oldMounts[mountID] then
            WCollections:GetMountNeedFanfareContainer()[mountID] = true;
        end
    end
...
function C_MountJournal.ClearFanfare(mountID)
    WCollections:GetMountNeedFanfareContainer()[mountID] = nil;
end

function C_MountJournal.ClearRecentFanfares()
    table.wipe(WCollections:GetMountNeedFanfareContainer());
end
...
function C_MountJournal.GetCollectedFilterSetting(filterIndex)
    return not WCollections:GetCVarBitfield("mountJournalGeneralFilters", filterIndex);
end
...
function C_MountJournal.GetIsFavorite(mountIndex)
    local mountID = _filteredMountIDs[mountIndex];
    local isFavorite = mountID and WCollections:GetMountFavoritesContainer()[mountID] and true or false;
...
function C_MountJournal.GetMountInfoByID(mountID)
    ...
    local flags;
    _, _, flags, name, _, sourceType, _, faction, usableFunc = WCollections:GetMountInfo(mountID);
    isFavorite = WCollections:GetMountFavoritesContainer()[mountID] and true or false;
    ...
    if companionIndex then
        ...
        isUsable = not WCollections.Config.Wardrobe.MountsUnusableInZone or IsMountUsable(mountID);
        isCollected = true;
    else
        spellID = mountID;
...
function C_MountJournal.GetMountInfoExtraByID(mountID)
    ...
    local flags;
    creatureDisplayInfoID, _, flags, _, description, _, source = WCollections:GetMountInfo(mountID);
...
function C_MountJournal.IsSourceChecked(filterIndex)
    return not WCollections:GetCVarBitfield("mountJournalSourcesFilter", filterIndex);
end

function C_MountJournal.IsTypeChecked(filterIndex)
    return not WCollections:GetCVarBitfield("mountJournalTypeFilter", filterIndex);
end

function C_MountJournal.NeedsFanfare(mountID)
    return WCollections:GetMountNeedFanfareContainer()[mountID];
end
...
function C_MountJournal.SetAllSourceFilters(isChecked)
    for filterIndex = 1, C_PetJournal.GetNumPetSources() do
        WCollections:SetCVarBitfield("mountJournalSourcesFilter", filterIndex, not isChecked);
    end
    SearchUpdated();
end
...
function C_MountJournal.SetAllTypeFilters(isChecked)
    for filterIndex = 1, 3 do
        WCollections:SetCVarBitfield("mountJournalTypeFilter", filterIndex, not isChecked);
    end
    SearchUpdated();
end

function C_MountJournal.SetCollectedFilterSetting(filterIndex, isChecked)
    WCollections:SetCVarBitfield("mountJournalGeneralFilters", filterIndex, not isChecked);
    SearchUpdated();
end
...
function C_MountJournal.SetIsFavorite(mountIndex, isFavorite)
    local mountID = _filteredMountIDs[mountIndex];
    if mountID then
        WCollections:GetMountFavoritesContainer()[mountID] = isFavorite and true or nil;
    end
    SearchUpdated();
end
...
function C_MountJournal.SetDefaultFilters()
    WCollections:SetCVar("mountJournalGeneralFilters", 0);
    WCollections:SetCVar("mountJournalSourcesFilter", 0);
    WCollections:SetCVar("mountJournalTypeFilter", 0);
    SearchUpdated();
end

function C_MountJournal.IsUsingDefaultFilters()
    return WCollections:GetCVar("mountJournalGeneralFilters") == 0
        and WCollections:GetCVar("mountJournalSourcesFilter") == 0
        and WCollections:GetCVar("mountJournalTypeFilter") == 0;
end
...
function C_MountJournal.SummonByID(mountID)
    ...
    if isUsable and withScaling then
        WCollections:SendAddonMessage("MOUNT:SCALINGCAST:"..mountID);
        return;
    end
