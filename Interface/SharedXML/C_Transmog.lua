...
local function CreateTransmogKey()
    ...
    for slot, transmogs in pairs(_pending) do
        ...
        local fakeEntry, fakeEnchantName, fakeEnchant = WCollections:GetItemTransmog("player", slot);
        ...
        if baseEntry then
            ...
            local entryOK = not pendingEntry or pendingEntry == 0 or pendingEntry == WCollections:GetHiddenVisualItem() or WCollections:HasAvailableSkin(pendingEntry);
            local enchantOK = not pendingEnchant or pendingEnchant == 0 or pendingEnchant == WCollections:GetHiddenVisualItem() or WCollections:HasAvailableSkin(pendingEnchant);
            ...
            if entryOK and enchantOK and pendingEnchant and pendingEnchant ~= 0 then
                ...
                local info = WCollections:GetSkinInfo(targetEntry);
...
function C_Transmog.ClearPending(slot, transmogType)
    ...
    if not _suppressUpdates then
        WCollections:RaiseEvent("TRANSMOGRIFY_UPDATE", slot, transmogType);
    end
end
...
function C_Transmog.ApplyAllPending(currentSpecOnly)
    ...
    WCollections:SendAddonMessage("TRANSMOGRIFY:APPLY:" .. key);
    return true;
end
        ...
        for i, slotString in ipairs({ strsplit(":", key) }) do
            if i == 1 then
                ...
                if tonumber(pendingEntry) ~= 0 then
                    WCollections:RaiseEvent("TRANSMOGRIFY_SUCCESS", tonumber(slot), LE_TRANSMOG_TYPE_APPEARANCE);
                end
                if tonumber(pendingEnchant) ~= 0 then
                    WCollections:RaiseEvent("TRANSMOGRIFY_SUCCESS", tonumber(slot), LE_TRANSMOG_TYPE_ILLUSION);
                end
            end
        end
...
function C_Transmog.GetCost()
    ...
    if _lastRequestedCost.Key ~= key then
        ...
        table.wipe(_pendingFailReasons);
        WCollections:SendAddonMessage("TRANSMOGRIFY:COST:" .. key);
    end
...
function C_Transmog.GetSlotInfo(slot, transmogType)
    ...
    if topmost == 0 or topmost == applied and applied == WCollections:GetHiddenVisualItem() then topmost = base; end

    local isTransmogrified = applied ~= 0;
    local hasPending = pending ~= 0;
    local isPendingCollected = pending and WCollections:HasAvailableSkin(pending) or pending == WCollections:GetHiddenVisualItem() or false;
    ...
    local isHideVisual = (hasPending and pending or applied) == WCollections:GetHiddenVisualItem();
    local texture = topmost ~= 0 and WCollections:GetSkinIcon(topmost) or nil;
...
function C_Transmog.LoadSources(sources, mainHandEnchant, offHandEnchant, clearExtra)
    ...
    if clearExtra and WCollections.Config.Wardrobe.HideExtraSlotsOnSetSelect then
        for _, slotInfo in ipairs(TRANSMOG_SLOTS) do
            if slotInfo.transmogType == LE_TRANSMOG_TYPE_APPEARANCE and slotInfo.armorCategoryID then
                ...
                if not sources[slot] and baseSourceID ~= 0 and appliedSourceID ~= WCollections:GetHiddenVisualItem() and WCollections:CanHideSlot(slotInfo.slot:gsub("SLOT", "")) then
                    C_Transmog.SetPending(slot, LE_TRANSMOG_TYPE_APPEARANCE, WCollections:GetHiddenVisualItem());
                end
            end
        end
    end
...
function C_Transmog.GetSlotVisualInfo(slot, transmogType)
    ...
    elseif transmogType == LE_TRANSMOG_TYPE_ILLUSION then
        local _, id, enchant = strsplit(":", GetInventoryItemLink("player", slot) or "");
        base = WCollections:GetScrollFromEnchant(tonumber(enchant) or 0) or 0;
        fake = fakeEnchant == WCollections:GetHiddenEnchant() and WCollections:GetHiddenVisualItem() or WCollections:GetScrollFromEnchant(fakeEnchant or 0) or 0;
    end
