...
function SetAtlasMixin:SetAtlas(atlasName, useAtlasSize)
	local atlasMember = WCollections.AtlasMember[atlasName:lower()];
	if atlasMember then
		local atlasID, left, right, top, bottom = unpack(atlasMember);
		local atlas = WCollections.Atlas[atlasID];
		if atlas then
...
function GameTooltip:SetTransmogrifyItem(slotID)
	...
	WCollections:SetPendingTooltipInfo("SetTransmogrifyItem", hasPendingUndo, pendingSourceID, hasPendingIllusionUndo, pendingIllusionID);
	self:SetInventoryItem("player", slotID);
	WCollections:ClearPendingTooltipInfo("SetTransmogrifyItem");
end

function GameTooltip:SetToyByItemID(itemID)
	WCollections:SetPendingTooltipInfo("SetToyByItemID");
	self:SetHyperlink("item:"..itemID);
	if self:IsShown() then
		local toyID = WCollections:GetToyIDByItem(itemID);
		local _, flags, _, _, sourceText, holiday = WCollections:GetToyInfo(toyID or 0);
		local lines = { };
		local reqs = { };
		local footer = { };
		...
		for i = 1, 20 do
			...
			if line and line:IsShown() and line:GetText() and line:GetText() ~= "" and line:GetText() ~= " " then
				...
				elseif WCollections.IsSameColor(r, g, b, 0, 1, 0) and text:find(ITEM_SPELL_TRIGGER_ONUSE) == 1 then -- Spell
				...
				elseif WCollections.IsSameColor(r, g, b, 0, 1, 0) and text:find(ITEM_SPELL_TRIGGER_ONEQUIP) == 1 and onEquipTriggers then -- Spell
				...
				elseif text:match(WCollections.FormatToPattern(ITEM_COOLDOWN_TIME)) then
					table.insert(lines, { text, r, g, b, 0 }); foundCooldown = true;
				elseif WCollections.IsSameColor(r, g, b, 1, 0.8235, 0) and text:sub(1, 1) == "\"" then -- Description
					table.insert(footer, { text, r, g, b, 1 });
				elseif WCollections.IsSameColor(r, g, b, 1, 0.125, 0.125) then -- Failed requirements
					table.insert(reqs, { text, r, g, b, 0 });
				elseif text:match(WCollections.FormatToPattern(ITEM_MIN_LEVEL))
					or text:match(WCollections.FormatToPattern(ITEM_MIN_SKILL))
					or text:match(WCollections.FormatToPattern(ITEM_LEVEL_RANGE))
					or text:match(WCollections.FormatToPattern(ITEM_LEVEL_RANGE_CURRENT))
					or text:match(WCollections.FormatToPattern(ITEM_REQ_REPUTATION))
					or text:match(WCollections.FormatToPattern(ITEM_REQ_SKILL)) then
					local wrap = 1;
					if holiday and text == format(ITEM_REQ_SKILL, WCollections.Holidays[holiday] or "") and not WCollections:IsHolidayActive(holiday) then
		...
		if not foundCooldown then
			...
			if start + duration >= now then
				local text = WCollections:FormatItemCooldown(math.floor((start + duration - now) * 1000));
				if text then
		...
		local hasToy = WCollections:HasToy(toyID);
		if not hasToy and WCollections:IsActiveToySubscriptionToy(toyID) then
			self:AddLine(" ");
			self:AddLine(WCollections.L["Toy.Subscription.Details.Info"], HIGHLIGHT_FONT_COLOR.r, HIGHLIGHT_FONT_COLOR.g, HIGHLIGHT_FONT_COLOR.b, 1);
		end
		if not hasToy or WCollections.Config.Wardrobe.ShowCollectedToySourceText then
			if sourceText ~= "" then
				self:AddLine(" ");
	...
	WCollections:ClearPendingTooltipInfo("SetToyByItemID");
	return self:IsShown();
end
