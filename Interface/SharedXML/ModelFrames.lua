...
function Model_OnMouseWheel(self, delta, maxZoom, minZoom)
	...
	zoomLevel = zoomLevel + delta * MODELFRAME_ZOOM_STEP * WCollections.Config.Wardrobe.CameraZoomSpeed;
	zoomLevel = min(zoomLevel, maxZoom);
	zoomLevel = max(zoomLevel, minZoom);
	if not WCollections.Config.Wardrobe.CameraZoomSmooth then
...
function Model_OnUpdate(self, elapsedTime, rotationsPerSecond)
	...
	if WCollections.Config.Wardrobe.CameraOption ~= self.cameraOption then
		self.settings = SelectSettings(self);
		self.portraitZoomMultiplier = self.settings.zoomMultiplier;
		self.minZoom = self.settings.minZoom or self.minZoom;
	end

	if WCollections.Config.Wardrobe.CameraZoomSmooth and self.zoomLevel then
		local x, y, z = self:GetPosition();
		local tx = self.zoomLevel * self.portraitZoomMultiplier * self:GetModelScale();
		local delta = tx - x;
		if math.abs(delta) > 0.001 then
			self.cameraX = x + delta * elapsedTime * 20 * WCollections.Config.Wardrobe.CameraZoomSmoothSpeed;
	...
	elseif ( self.panning ) then
		...
		-- bounds
		if WCollections.Config.Wardrobe.CameraPanLimit then
...
function GetUICameraInfo(cameraID)
	...
	local camera;
	if cameraID >= 0 then
		camera = WCollections.Cameras[cameraID];
	else
		camera = WCollections.Cache.Cameras[-cameraID];
	end
	if camera then
