...
local function LinkOutfitDropDownInit()
	...
	info.func = function()
		...
		StaticPopup_Show("WCOLLECTIONS_COPY_OUTFIT_COMMAND", nil, nil, slashCommand);
...
function DressUpModelFrameLinkButtonMixin:OnLoad()
	WCollections:UIDropDownMenu_Initialize(self.DropDown, LinkOutfitDropDownInit, "MENU");
end
...
function DressUpModelFrameBaseMixin:SetMode(mode)
	...
	if self.hasOutfitControls then
		...
		if not inPlayerMode then
			...
			self:SetShownOutfitDetailsPanel(WCollections:GetCVarBool("showOutfitDetails"));
		end
	end
end
...
function DressUpModelFrameMixin:ToggleOutfitDetails()
	...
	WCollections:SetCVar("showOutfitDetails", show);
end
