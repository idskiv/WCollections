...
local function MatchesFilter(petID)
    ...
    local _, _, flags, _, _, sourceType = WCollections:GetPetInfo(petID);
    local name, _, petType = C_PetJournal.GetPetInfoBySpeciesID(petID);

    if WCollections:IsActivePetSubscriptionPet(petID) then
        if not _showSubscription then
            return false;
        end
    elseif not (_showCollected and isCollected or _showUncollected and not isCollected) then
        return false;
    elseif bit.band(flags or 0, 0x20) ~= 0 and not isCollected and not WCollections.Config.Wardrobe.PetsShowHidden then
        return false;
    end
...
local function SelectRandomFavoritePet()
    ...
    for companionIndex = 1, GetNumCompanions("CRITTER") do
        ...
        if WCollections:GetPetFavoritesContainer()[petID] then
...
local function CreateFavoriteMacro(perChar)
    local macro = CreateMacro(WCollections.L["Macro.Pet"], FindMacroIcon([[Interface\Icons\Ability_Hunter_BeastCall]]) or 1, _macroBody, perChar);
    MacroFrame_Update();
    return macro;
end
...
local function ShowMacroPopup()
    LoadAddOn("Blizzard_MacroUI");
    StaticPopupDialogs["WCOLLECTIONS_PET_MACRO_CREATE"].OnAccept = function(self)
        C_Timer.After(0, function()
            if InCombatLockdown() then
                StaticPopup_Show("WCOLLECTIONS_ERROR", WCollections.L["Popup.Error.Pet.Macro.Combat"]);
                return;
            end
            ...
            elseif numChar < MAX_CHARACTER_MACROS then
                StaticPopup_Show("WCOLLECTIONS_PET_MACRO_PERCHARACTER");
            else
                StaticPopup_Show("WCOLLECTIONS_ERROR", WCollections.L["Popup.Error.Mount.Macro.NoSpace"]);
            end
        end);
    end;
    StaticPopupDialogs["WCOLLECTIONS_PET_MACRO_PERCHARACTER"].OnAccept = function(self)
        C_Timer.After(0, function()
            if InCombatLockdown() then
    ...
    StaticPopup_Show("WCOLLECTIONS_PET_MACRO_CREATE");
end
...
function C_PetJournal.RefreshPets() -- Custom
    ...
    for petID, info in pairs(WCollections.Pets) do
    ...
    for companionIndex = 1, GetNumCompanions("CRITTER") do
        ...
        if oldPets and not oldPets[petID] then
            WCollections:GetPetNeedFanfareContainer()[petID] = true;
        end
    end
...
function C_PetJournal.GetPetInfoBySpeciesID(speciesID)
    ...
    creatureID, petType, _, name, description, sourceType, sourceText = WCollections:GetPetInfo(petID);
...
function C_PetJournal.GetPetInfoByPetID(petGUID)
    ...
    speciesID = petID; -- Custom
    isFavorite = petID and WCollections:GetPetFavoritesContainer()[petID] and true or false;
...
function C_PetJournal.GetNumPetsNeedingFanfare()
    local count = 0;
    for petID in WCollections:GetPetNeedFanfareContainer() do
        count = count + 1;
    end
    return count;
end
...
function C_PetJournal.PetNeedsFanfare(petGUID)
    ...
    return WCollections:GetPetNeedFanfareContainer()[petID];
end

function C_PetJournal.ClearRecentFanfares()
    table.wipe(WCollections:GetPetNeedFanfareContainer());
end
...
function C_PetJournal.ClearFanfare(petGUID)
    ...
    WCollections:GetPetNeedFanfareContainer()[petID] = nil;
end
...
function C_PetJournal.SetFavorite(petID, bool)
    ...
    if petID then
        WCollections:GetPetFavoritesContainer()[petID] = bool and true or nil;
    end
    SearchUpdated();
end
...
function C_PetJournal.PetIsFavorite(ID)
    ...
    local petID = _filteredPetIDs[petIndex];
    local isFavorite = petID and WCollections:GetPetFavoritesContainer()[petID] and true or false;
...
function C_PetJournal.SetPetSortParameter(parameterID)
    WCollections:SetCVar("petJournalSort", parameterID);
    SearchUpdated();
end

function C_PetJournal.GetPetSortParameter()
    return WCollections:GetCVar("petJournalSort");
end
...
function C_PetJournal.SetPetTypeFilter(petTypeIndex, value)
    WCollections:SetCVarBitfield("petJournalTypeFilters", petTypeIndex, not value);
    SearchUpdated();
end

function C_PetJournal.IsPetTypeChecked(petTypeIndex)
    return not WCollections:GetCVarBitfield("petJournalTypeFilters", petTypeIndex);
end

function C_PetJournal.SetAllPetTypesChecked(checked)
    for filterIndex = 1, C_PetJournal.GetNumPetTypes() do
        WCollections:SetCVarBitfield("petJournalTypeFilters", filterIndex, not checked);
    end
    SearchUpdated();
end
...
function C_PetJournal.SetPetSourceChecked(petSourceIndex, value)
    WCollections:SetCVarBitfield("petJournalSourceFilters", petSourceIndex, not value);
    SearchUpdated();
end

function C_PetJournal.IsPetSourceChecked(petSourceIndex)
    return not WCollections:GetCVarBitfield("petJournalSourceFilters", petSourceIndex);
end

function C_PetJournal.SetAllPetSourcesChecked(checked)
    for filterIndex = 1, C_PetJournal.GetNumPetSources() do
        WCollections:SetCVarBitfield("petJournalSourceFilters", filterIndex, not checked);
    end
    SearchUpdated();
end

function C_PetJournal.SetFilterChecked(filter, value)
    WCollections:SetCVarBitfield("petJournalFilters", filter, not value);
    SearchUpdated();
end

function C_PetJournal.IsFilterChecked(filter)
    return not WCollections:GetCVarBitfield("petJournalFilters", filter);
end

function C_PetJournal.SetDefaultFilters()
    WCollections:SetCVar("petJournalFilters", 0);
    WCollections:SetCVar("petJournalSourceFilters", 0);
    WCollections:SetCVar("petJournalTypeFilters", 0);
    SearchUpdated();
end

function C_PetJournal.IsUsingDefaultFilters()
    return WCollections:GetCVar("petJournalFilters") == 0
        and WCollections:GetCVar("petJournalSourceFilters") == 0
        and WCollections:GetCVar("petJournalTypeFilters") == 0;
end
