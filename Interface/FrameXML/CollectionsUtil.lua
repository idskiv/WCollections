...
function CollectionWardrobeUtil.GetAppearanceNameTextAndColor(appearanceInfo, inLegionArtifactCategory)
	...
	elseif appearanceInfo.name then
		...
		WCollections:QueryItem(appearanceInfo.sourceID);
...
function CollectionWardrobeUtil.GetAppearanceSourceTextAndColor(appearanceInfo, forceForCollected)
	...
	if appearanceInfo.isCollected and not forceForCollected then
		text = WCollections.L["Tooltip.Transmog.Collected"]; -- TRANSMOG_COLLECTED
...
function CollectionWardrobeUtil.AddSourceSetsToTooltip(tooltip, sourceID)
	...
	local tooltipSourceSlotCollected = WCollections:HasAvailableSkin(sourceID);
	if info and info.Sets then
		local color = WCollections.Config.TooltipSets.Color;
		if WCollections.Config.TooltipSets.Separator then tooltip:AddLine(" "); end

		if #info.Sets > 1 and SHOW_PLURAL_SETS_HEADER then
			tooltip:AddLine(format(WCollections.L["Tooltip.Sets.Header.Plural"], #info.Sets), color.r * 0.75, color.g * 0.75, color.b * 0.75, false);
		end
		...
		for _, setID in ipairs(info.Sets) do
			...
			for source, collected in pairs(C_TransmogSets.GetSetSources(set.setID, true)) do
				...
				if not tooltipSourceSlotCollected and collected then
					local info = WCollections:GetSkinInfo(source);
			...
			if WCollections.Config.TooltipSets.SlotStateStyle > 1 then
				if slotCollectedFromAnotherSource then
					state = "KnownFromAnotherSource";
					iconTexture = [[Interface\AddOns\WCollections\Textures\TooltipIcons\KNOWN_circle]];
				elseif tooltipSourceSlotCollected then
					state = "Known";
					iconTexture = [[Interface\AddOns\WCollections\Textures\TooltipIcons\KNOWN]];
				else
					state = "Unknown";
					iconTexture = [[Interface\AddOns\WCollections\Textures\TooltipIcons\UNKNOWN]];
				end

				local line = _G[tooltip:GetName().."TextLeft"..tooltip:NumLines()];
				local oldText = line:GetText();
				line:SetText([[|TInterface\AddOns\WCollections\Interface\Common\spacer:100:1|t]]);
				...
				if WCollections.Config.TooltipSets.SlotStateStyle == 2 then
			...
			tooltip:AddDoubleLine(
				format(WCollections.L[(#info.Sets == 1 or not SHOW_PLURAL_SETS_HEADER) and "Tooltip.Sets.Header.Singular" or "Tooltip.Sets.Set"],
					format("|cFF%s%s|r",
						WCollections.RGBPercToHex(color.r, color.g, color.b),
						set.name),
			...
			if WCollections.Config.TooltipSets.SlotStateStyle == 3 or (WCollections.Config.TooltipSets.SlotStateStyle == 4 and slotCollectedFromAnotherSource) then
				tooltip:AddDoubleLine(" ", format("|T%s:%g|t %s", iconTexture, iconHeight, WCollections.L["TooltipSets.SlotState."..state]));
			end
		end
		return true;
	end
...
function CollectionWardrobeUtil.GetAppearanceSourceTextWithDrops(appearanceInfo, sourceText)
	...
	local appearanceCollected = appearanceInfo.isCollected and not WCollections.Config.Wardrobe.ShowCollectedVisualSourceText;
...
function CollectionWardrobeUtil.SetAppearanceTooltip(tooltip, sources, primarySourceID, selectedIndex, showUseError, inLegionArtifactCategory, subheaderString, showSets)
	...
	local sourceText, sourceColor = CollectionWardrobeUtil.GetAppearanceSourceTextAndColor(sources[headerIndex], WCollections.Config.Wardrobe.ShowCollectedVisualSourceText);
	GameTooltip_SetTitle(tooltip, name, nameColor);
	if subheaderString then
		GameTooltip_AddHighlightLine(tooltip, subheaderString);
	end

	local info = WCollections:GetSkinInfo(headerSourceID);
	if info and info.RaceMask then
		local allianceRaceMask = 0;
		local hordeRaceMask = 0;
		for race, faction in pairs(WCollections.RaceNameToFaction) do
			if faction == FACTION_ALLIANCE then
				allianceRaceMask = bit.bor(allianceRaceMask, bit.lshift(1, WCollections.RaceNameToID[race] - 1));
			elseif faction == FACTION_HORDE then
				hordeRaceMask = bit.bor(hordeRaceMask, bit.lshift(1, WCollections.RaceNameToID[race] - 1));
			end
		end
		...
		elseif info.RaceMask == hordeRaceMask then
			...
			local races = "";
			local match = false;
			for i, name in ipairs(WCollections.RaceIDToName) do
				if bit.band(info.RaceMask, bit.lshift(1, i - 1)) ~= 0 then
					races = races .. (races ~= "" and ", " or "") .. WCollections.L["Race."..i];
	...
	if info and info.ClassMask then
		local classes = "";
		local match = false;
		for i, name in ipairs(WCollections.ClassIDToName) do
	...
	elseif WCollections.Config.Wardrobe.ShowItemID and not C_TransmogCollection.IsAppearanceHiddenVisual(headerSourceID) then
	...
	local useError;
	if ( #sources > 1 and (not appearanceCollected or WCollections.Config.Wardrobe.ShowCollectedVisualSources) ) then
		...
		local text;
		if WCollections.Config.Wardrobe.TooltipCycleKeyboard and WCollections.Config.Wardrobe.TooltipCycleMouseWheel then
			text = WCollections.L["Tooltip.Cycle.KeyboardAndMouseWheel"];
		elseif WCollections.Config.Wardrobe.TooltipCycleKeyboard then
			text = WCollections.L["Tooltip.Cycle.Keyboard"];
		elseif WCollections.Config.Wardrobe.TooltipCycleMouseWheel then
			text = WCollections.L["Tooltip.Cycle.MouseWheel"];
		end
		if text then
			GameTooltip_AddBlankLineToTooltip(tooltip);
