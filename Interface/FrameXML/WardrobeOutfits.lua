...
function WardrobeOutfitDropDownTemplate_OnShow(self)
	...
	WCollections:RegisterEvent(self, "TRANSMOG_OUTFITS_CHANGED");
...
function WardrobeOutfitDropDownTemplate_OnHide(self)
	...
	WCollections:UnregisterEvent(self, "TRANSMOG_OUTFITS_CHANGED");
...
function WardrobeOutfitFrameMixin:Update()
	...
	local sort = WCollections.Config.Wardrobe.OutfitsSort;
	...
	for i = 1, C_TransmogCollection.GetNumMaxOutfits() do
		...
		if ( outfits[i] or newOutfitButton ) then
			...
			if ( newOutfitButton ) then
				...
				button.Icon:SetTexture([[Interface\AddOns\WCollections\Interface\PaperDollInfoFrame\Character-Plus]]);
				button.outfitID = nil;
				button.Check:Hide();
				...
				if button.prepaid and WCollections.Config.Wardrobe.OutfitsPrepaidSheen then
	...
	self:SetWidth(stringWidth + OUTFIT_FRAME_ADDED_PIXELS + (WCollections.PrepaidOutfitsEnabled and 14 or 0));
...
function WardrobeOutfitButtonTemplate_OnClick(self)
	...
	if ( self.outfitID ) then
		...
		if ( WardrobeTransmogFrame and WardrobeTransmogFrame.OutfitHelpBox:IsShown() ) then
			...
			WCollections:SetCVarBitfield("closedInfoFrames", LE_FRAME_TUTORIAL_TRANSMOG_OUTFIT_DROPDOWN, true);
		end
