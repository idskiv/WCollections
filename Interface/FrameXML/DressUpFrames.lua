...
local function TransformTransmogLinkToItemLink(link)
	...
	elseif strsub(link, 1, 16) == "transmogillusion" then
		...
		local enchant = WCollections:GetEnchantFromScroll(tonumber(sourceID) or 0);
	...
	elseif strsub(link, 1, 18) == "transmogappearance" then
		...
		if itemID and WCollections:GetEnchantFromScroll(itemID) then
			return TransformTransmogLinkToItemLink("transmogillusion:"..itemID);
		end
		local dressable = itemID and WCollections:GetDressableFromRecipe(itemID);
		if dressable then
...
WCollectionsDressUpItemLink = DressUpItemLink;
...
local function GetFrameAndSetBackground(raceFilename, classFilename)
	...
	elseif AuctionDressUpFrame and ShouldAcceptDressUp(AuctionDressUpFrame, AuctionFrame) then
		...
		if dressUpAddonEnabled and WCollections.Config.Wardrobe.DressUpClassBackground then
...
if ADDON_NAME ~= "WCollectionsDressUp" then
...
DressUpModel = nil;
function WCollectionsAuctionOnShowHook(self)
	SetUpSideDressUpFrame(self, 840, 1020, "TOPLEFT", "TOPRIGHT", -2, -28);
end
function WCollectionsAuctionOnHideHook(self)
...
function DressUpTexturePath(raceFileName)
	...
	if ( strupper(fileName) == "GNOME" ) then
		if WCollections.Config.Wardrobe.DressUpGnomeTrollBackground then
			return [[Interface\AddOns\WCollections\Interface\DressUpFrame\DressUpBackground-Gnome]];
		end
		raceFileName = "Dwarf";
	elseif ( strupper(fileName) == "TROLL" ) then
		if WCollections.Config.Wardrobe.DressUpGnomeTrollBackground then
			return [[Interface\AddOns\WCollections\Interface\DressUpFrame\DressUpBackground-Troll]];
		end
		raceFileName = "Orc";
	end
	if ( not raceFileName ) then
...
function SetDressUpBackground(frame, fileName, atlasPostfix)
	...
	if dressUpAddonEnabled then
		if frame.ModelBackground then frame.ModelBackground:SetDesaturated(WCollections.Config.Wardrobe.DressUpDesaturateBackground); end
		if frame.BGTopLeft then frame.BGTopLeft:SetDesaturated(WCollections.Config.Wardrobe.DressUpDesaturateBackground); end
		if frame.BGTopRight then frame.BGTopRight:SetDesaturated(WCollections.Config.Wardrobe.DressUpDesaturateBackground); end
		if frame.BGBottomLeft then frame.BGBottomLeft:SetDesaturated(WCollections.Config.Wardrobe.DressUpDesaturateBackground); end
		if frame.BGBottomRight then frame.BGBottomRight:SetDesaturated(WCollections.Config.Wardrobe.DressUpDesaturateBackground); end
	end
end

local function SetupDressUpModel(model)
	local x, y, z = GetUICameraInfo(WCollections:GetCharacterCameraID("DressUp"));
	if not x then
		x, y = GetUICameraInfo(WCollections:GetCharacterCameraID("SetsDetails"));
		z = 0;
	end
...
function DressUpModel_OnLoad(self)
	...
	function self:InitSources()
		...
		self.conflictingWeapons = WCollections:AreWeaponsImpossibleToDisplayInPlayerModel(self.sources[16], self.sources[17]);
	...
	function self:UndressSlot(slot, illusion)
		...
		if not illusion then
			...
			self.conflictingWeapons = WCollections:AreWeaponsImpossibleToDisplayInPlayerModel(sources[16], sources[17]);
		...
		if id then
			...
			if invType or C_TransmogCollection.IsAppearanceHiddenVisual(id) and slot then
				...
				elseif invType == 13 or invType == 14 or invType == 15 or invType == 17 or invType == 21 or invType == 22 or invType == 23 or invType == 25 or invType == 26 then
					-- INVTYPE_WEAPON
					if invType == 13 then
						local mainHandInvType = sources[16] and WCollections:GetInvType(sources[16]);
						--if not sources[16] or mainHandInvType ~= 13 and mainHandInvType ~= 21 and mainHandInvType ~= 17 or not isLocalPlayerModel or not WCollections:PlayerCanDualWield() then
						if sources[16] and (mainHandInvType == 13 or mainHandInvType == 21 or mainHandInvType == 17) and isLocalPlayerModel and WCollections:PlayerCanDualWield() then
					...
					if invType == 17 or invType == 21 then
						if WCollections:CanEquipItemIntoSlot(id, 17) then
							slot = self.nextWeaponSlot;
							self.nextWeaponSlot = (self.nextWeaponSlot == 16 and 1 or 0) + 16;
						else
							slot = 16;
					...
					if slot == 16 or slot == 17 then
						...
						if sources[otherSlot] then
							if WCollections:AreWeaponsImpossibleToDisplayInPlayerModel(sources[16], sources[17]) then
				...
				if sources[slot] then
					if slot == 16 then
						self.mainHandEnchant = enchant == WCollections:GetHiddenEnchant() and WCollections:GetHiddenVisualItem() or enchant and WCollections:GetScrollFromEnchant(enchant);
						self.displayedWeapon = self.conflictingWeapons and 3 or 1;
					elseif slot == 17 then
						self.offHandEnchant = enchant == WCollections:GetHiddenEnchant() and WCollections:GetHiddenVisualItem() or enchant and WCollections:GetScrollFromEnchant(enchant);
	...
	function self:RefreshDressFromSources()
		...
		mainHandEnchant = mainHandEnchant and WCollections:GetEnchantFromScroll(mainHandEnchant);
		offHandEnchant = offHandEnchant and WCollections:GetEnchantFromScroll(offHandEnchant);
		...
		if self.displayedWeapon == 1 then
			...
			if WCollections:AreWeaponsImpossibleToDisplayInPlayerModel(mh, oh) then
				...
				local mhInvType = mh and WCollections:GetInvType(mh);
				local ohInvType = oh and WCollections:GetInvType(oh);
...
function DressUpFrame_Show(frame, model)
	...
	if ( not frame:IsShown() or frame:GetMode() ~= "player") then
		...
		ShowUIPanel(frame);
		if frame == DressUpFrame and UIPanelWindows["DressUpFrame"] and WCollections.Config.Windows["DressUpFrame"].Layout and UIPanelNotShown(frame) then
			HideUIPanel(frame);
		...
		Model_Reset(model);
		if not WCollections.Config.Wardrobe.DressUpSkipDressOnShow then
			model:Dress();
		end
	end
end
...
function DressUpOutfitDetailsPanelMixin:OnShow()
	...
	WCollections:RegisterEvent(self, "TRANSMOG_SOURCE_COLLECTABILITY_UPDATE");
	self:Refresh();
end

function DressUpOutfitDetailsPanelMixin:OnHide()
	WCollections:UnregisterEvent(self, "TRANSMOG_COLLECTION_ITEM_UPDATE");
	WCollections:UnregisterEvent(self, "TRANSMOG_SOURCE_COLLECTABILITY_UPDATE");
end
...
local TRANSMOGRIFY_TOOLTIP_APPEARANCE_KNOWN_CHECKMARK = [[|TInterface\AddOns\WCollections\interface\Common\CommonIcons:16:16:0:-1:2048:1024:1:257:517:773|t ]]..TRANSMOGRIFY_TOOLTIP_APPEARANCE_KNOWN;
...
function DressUpOutfitDetailsSlotMixin:OnEnter()
	...
	elseif not self.item then
		...
		if WCollections.Config.Wardrobe.ShowItemID then
...
function DressUpOutfitDetailsSlotMixin:SetAppearance(slotID, transmogID)
	...
	if not itemID then
		...
		if not GetItemInfo(self.item) then
			...
			WCollections:QueryItem(self.item);
		end
...
function DressUpOutfitDetailsSlotMixin:SetItemInfo(transmogID, appearanceInfo, slotID)
	...
	elseif appearanceInfo.appearanceIsCollected then
		...
		if isHiddenVisual then
			name = WCollections:GetHiddenVisualItemName(slotID);
...
function DressUpOutfitDetailsSlotMixin:SetIllusion(transmogID)
	...
	if isHiddenVisual then
		icon = [[Interface\AddOns\WCollections\Textures\EnchantSlotIcon]];
	end
