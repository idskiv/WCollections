...
local function UIDropDownMenu_Initialize(frame, initFunction, displayMode, level, menuList)
	WCollections:UIDropDownMenu_Initialize(frame, initFunction, displayMode, level, menuList);
end
...
function WardrobeTransmogFrame_OnLoad(self)
	...
	WCollections:RegisterEvent(self, "TRANSMOGRIFY_UPDATE");
	WCollections:RegisterEvent(self, "TRANSMOGRIFY_ITEM_UPDATE");
	WCollections:RegisterEvent(self, "TRANSMOGRIFY_SUCCESS");
	...
	function self.Model:TryOn(appearanceSourceID, slot, illusionSourceID)
		local enchant = WCollections:GetEnchantFromScroll(illusionSourceID);
		if enchant then
...
function WardrobeTransmogFrame_OnEvent(self, event, ...)
	if ( event == "TRANSMOGRIFY_UPDATE" or event == "TRANSMOGRIFY_ITEM_UPDATE" ) then
		...
		if ( slotButton ) then
			...
			if ( hasUndo ) then
				PlaySoundFile([[Interface\AddOns\WCollections\Sounds\UI_VoidStorage_Undo.wav]]);
			elseif ( not hasPending ) then
				if ( slotButton.hadUndo ) then
					PlaySoundFile([[Interface\AddOns\WCollections\Sounds\UI_VoidStorage_Redo.wav]]);
					slotButton.hadUndo = nil;
				end
			end
			-- specs button tutorial
			if ( hasPending and not hasUndo ) then
				if ( not WCollections:GetCVarBitfield("closedInfoFrames", LE_FRAME_TUTORIAL_TRANSMOG_SPECS_BUTTON) ) and WCollections.Config.Wardrobe.OutfitsSelectLastUsed then
...
function WardrobeTransmogFrame_OnShow(self)
	...
	if WCollections.Config.Wardrobe.EtherealWindowSound then
		PlaySoundFile([[Interface\AddOns\WCollections\Sounds\UI_EtherealWindow_Open.wav]]);
	else
...
function WardrobeTransmogFrame_OnHide(self)
	if WCollections.Config.Wardrobe.EtherealWindowSound then
		PlaySoundFile([[Interface\AddOns\WCollections\Sounds\UI_EtherealWindow_Close.wav]]);
	else
...
function WardrobeTransmogFrame_Update()
	...
	if slot == "MAINHANDSLOT" or slot == "SECONDARYHANDSLOT" then
		...
		if WCollections:AreWeaponsImpossibleToDisplayInPlayerModel(mh, oh) then
			WardrobeTransmogFrame_UpdateWeaponModel(slot);
			WardrobeTransmogFrame.Model.WeaponHandWarning:Show();
		else
			local mhInvType = mh and mh ~= 0 and WCollections:GetInvType(mh);
			local ohInvType = oh and oh ~= 0 and WCollections:GetInvType(oh);
			if (mhInvType == 17 or mhInvType == 21) and not WCollections:CanEquipItemIntoSlot(mh, 17) then
...
function WardrobeTransmogFrame_UpdateSlotButton(slotButton)
	...
	if ( hasChange and (hasUndo or isPendingCollected and not slotButton.invalidWeapon) and not slotButton.AnimFrame:IsShown() ) then
		...
		if slotFailReason then
			...
			slotButton.PendingFrame.Ants:SetTexture([[Interface\AddOns\WCollections\Textures\RedIconAlertAnts]]);
			slotButton.failed = true;
		else
			slotButton.PendingFrame.Glow:SetVertexColor(1, 1, 1);
			slotButton.PendingFrame.Ants:SetTexture([[Interface\AddOns\WCollections\Interface\Transmogrify\PurpleIconAlertAnts]]);
		end
		if ( hasUndo ) then
	...
	if ( isHideVisual and not hasUndo and not slotButton.invalidWeapon ) then
		...
		local baseTexture;
		if slotButton.transmogType == LE_TRANSMOG_TYPE_APPEARANCE then
			baseTexture = (WCollectionsGetInventoryItemTexture or GetInventoryItemTexture)("player", slotID);
		else
...
function WardrobeTransmogFrame_UpdateApplyButton()
	...
	if tokens and tokens > 0 and WCollections.Token then
		AltCurrencyFrame_Update("WardrobeTransmogTokenFrame", select(10, GetItemInfo(WCollections.Token)), tokens);
		WardrobeTransmogTokenFrame:Show();
		if tokens > GetItemCount(WCollections.Token) then
...
function WardrobeTransmogFrame_ApplyPending(lastAcceptedWarningIndex)
	...
	if ( WardrobeTransmogFrame.applyWarningsTable and lastAcceptedWarningIndex < #WardrobeTransmogFrame.applyWarningsTable ) then
		...
		local success = C_Transmog.ApplyAllPending(WCollections:GetCVarBool("transmogCurrentSpecOnly"));
		if ( success ) then
			WardrobeTransmogFrame.applyWarningsTable = nil;
			-- outfit tutorial
			if ( not WCollections:GetCVarBitfield("closedInfoFrames", LE_FRAME_TUTORIAL_TRANSMOG_OUTFIT_DROPDOWN) ) then
...
function WardrobeTransmogFrame_OnTransmogApplied()
	PlaySoundFile([[Interface\AddOns\WCollections\Sounds\UI_Transmogrify_Apply.wav]]);
...
function WardrobeOutfitMixin:OnOutfitApplied(outfitID)
	...
	if WCollections:GetCVarBool("transmogCurrentSpecOnly") then
		local specIndex = GetSpecialization();
		WCollections:SetCVar("lastTransmogOutfitIDSpec"..specIndex, value);
	else
...
function WardrobeOutfitMixin:OnSelectOutfit(outfitID)
	...
	local value = outfitID or "";
	for specIndex = 1, GetNumSpecializations() do
		if WCollections:GetCVar("lastTransmogOutfitIDSpec"..specIndex) == "" then
			WCollections:SetCVar("lastTransmogOutfitIDSpec"..specIndex, value);
		end
	end
end
...
function WardrobeOutfitMixin:GetLastOutfitID()
	if not WCollections.Config.Wardrobe.OutfitsSelectLastUsed then return; end
	local specIndex = GetSpecialization();
	return tonumber(WCollections:GetCVar("lastTransmogOutfitIDSpec"..specIndex));
end
...
function WardrobeTransmogButton_OnClick(self, button)
	...
	if ( button == "RightButton" ) then
		if ( hasPending or hasUndo ) then
			PlaySoundFile([[Interface\AddOns\WCollections\Sounds\UI_Reforging_Restore.wav]]);
...
function WardrobeItemsCollectionMixin:OnLoad()
	...
	WCollections:RegisterEvent(self, "TRANSMOG_COLLECTION_UPDATED");
...
function WardrobeItemsCollectionMixin:ShouldShowSetsHelpTip()
	if (WardrobeFrame_IsAtTransmogrifier()) then
		if (WCollections:GetCVarBitfield("closedInfoFrames", LE_FRAME_TUTORIAL_TRANSMOG_SETS_VENDOR_TAB)) then
			return false;
		end

		if (not WCollections:GetCVarBitfield("closedInfoFrames", LE_FRAME_TUTORIAL_TRANSMOG_SPECS_BUTTON)) then
			return false;
		end
		...
		if (WCollections:GetCVarBitfield("closedInfoFrames", LE_FRAME_TUTORIAL_TRANSMOG_SETS_TAB)) then
			return false;
		end

		if (not WCollections:GetCVarBitfield("closedInfoFrames", LE_FRAME_TUTORIAL_TRANSMOG_MODEL_CLICK)) then
			return false;
		end
...
function WardrobeItemsCollectionMixin:OnShow()
	...
	-- tab tutorial
	WCollections:SetCVarBitfield("closedInfoFrames", LE_FRAME_TUTORIAL_TRANSMOG_JOURNAL_TAB, true);
...
function WardrobeItemsCollectionMixin:OnHide()
	WCollections:UnregisterEvent(self, "TRANSMOGRIFY_UPDATE");
	self:UnregisterEvent("PLAYER_EQUIPMENT_CHANGED");
	WCollections:UnregisterEvent(self, "TRANSMOGRIFY_SUCCESS");
...
function WardrobeCollectionFrame_OnShow(self)
	...
	WCollections:RegisterEvent(self, "TRANSMOG_COLLECTION_ITEM_UPDATE");
	self:RegisterEvent("UNIT_MODEL_CHANGED");
	WCollections:RegisterEvent(self, "TRANSMOG_SEARCH_UPDATED");
	WCollections:RegisterEvent(self, "SEARCH_DB_LOADED");
	...
	WCollections:RegisterEvent(self, "TRANSMOG_COLLECTION_CAMERA_UPDATE");
...
function WardrobeCollectionFrame_OnHide(self)
	WCollections:UnregisterEvent(self, "TRANSMOG_COLLECTION_ITEM_UPDATE");
	self:UnregisterEvent("UNIT_MODEL_CHANGED");
	WCollections:UnregisterEvent(self, "TRANSMOG_SEARCH_UPDATED");
	WCollections:UnregisterEvent(self, "SEARCH_DB_LOADED");
	...
	WCollections:UnregisterEvent(self, "TRANSMOG_COLLECTION_CAMERA_UPDATE");
...
function WardrobeItemsCollectionMixin:FilterVisuals()
	local missing = false;
	local showClaimable = WCollections:GetCVarBool("transmogrifyShowClaimable");
	local showPurchasable = WCollections:GetCVarBool("transmogrifyShowPurchasable");
	local showObtainable = not WCollections.Developer or WCollections:GetCVarBool("transmogrifyShowObtainable");
	local showUnobtainable = WCollections.Developer and WCollections:GetCVarBool("transmogrifyShowUnobtainable");
	...
	for i = 1, #visualsList do
		if ( isAtTransmogrifier ) then
			...
			elseif not visualsList[i].isCollected and showClaimable and WCollections:CanClaimSkin(visualsList[i].visualID) then
				tinsert(filteredVisualsList, visualsList[i]);
			...
			elseif visualsList[i].isCollected and not GetItemInfo(visualsList[i].visualID) then
				missing = true;
				WCollections:QueryItem(visualsList[i].visualID);
				if not WardrobeCollectionFrame.visualListCacheRefreshTimer then
					WardrobeCollectionFrame.visualListCacheRefreshTimer = WCollections.AceAddon:ScheduleRepeatingTimer(function()
						local missing = false;
						...
						if not missing then
							WCollections.AceAddon:CancelTimer(WardrobeCollectionFrame.visualListCacheRefreshTimer);
			...
			local hide;
			if WCollections.Developer then
				local info = WCollections:GetSkinInfo(visualsList[i].visualID);
				if info then
...
function WardrobeItemsCollectionMixin:UpdateItems()
	...
	local checkTutorialFrame = (self.transmogType == LE_TRANSMOG_TYPE_APPEARANCE) and not WardrobeFrame_IsAtTransmogrifier()
								and not WCollections:GetCVarBitfield("closedInfoFrames", LE_FRAME_TUTORIAL_TRANSMOG_MODEL_CLICK);
	...
	for i = 1, self.PAGE_SIZE do
		...
		if ( visualInfo ) then
			...
			if not hasItemData then
				WCollections:QueryItem(visualInfo.visualID);
			end
			if self.transmogType == LE_TRANSMOG_TYPE_ILLUSION then
			...
			model.ClaimQuest:SetShown(not visualInfo.isCollected and WCollections:CanClaimSkin(visualInfo.visualID));
			...
			model.SubscriptionOverlay:SetShown(visualInfo.isCollected and not WCollections:HasSkin(visualInfo.visualID) and WCollections:GetActiveSubscriptionForSkin(visualInfo.visualID));
			...
			if hasItemData then
				...
				if not self.itemCacheRefreshTimer then
					self.itemCacheRefreshTimer = WCollections.AceAddon:ScheduleRepeatingTimer(function()
						local missing = false;
						if self:IsVisible() then
							local refresh = false;
						...
						if not missing then
							WCollections.AceAddon:CancelTimer(self.itemCacheRefreshTimer);
	...
	if ( checkTutorialFrame ) then
		if ( C_TransmogCollection.HasFavorites() ) then
			WCollections:SetCVarBitfield("closedInfoFrames", LE_FRAME_TUTORIAL_TRANSMOG_MODEL_CLICK, true);
...
function WardrobeItemsCollectionMixin:RefreshAppearanceTooltip()
	...
	WardrobeCollectionFrame_SetAppearanceTooltip(self, sources, chosenSourceID, WCollections.Config.Wardrobe.ShowSetsInAppearances);
end
...
function TransmogModelMixin:SetType(type, force)
	...
	elseif self.type == "main" then
		...
		self:SetCreature(WCollections.CreatureWeaponPreview);
...
function TransmogModelMixin:SetItemAppearance(visualID, enchant)
	...
	enchant = enchant and WCollections:GetEnchantFromScroll(enchant) or nil;
	if enchant then
...
function WardrobeItemsModelMixin:OnMouseDown(button)
	...
	elseif ( IsModifiedClick("DRESSUP") ) then
		...
		elseif ( transmogType == LE_TRANSMOG_TYPE_ILLUSION ) then
			...
			local enchant = WCollections:GetEnchantFromScroll(self.visualInfo.sourceID);
			if C_TransmogCollection.IsAppearanceHiddenVisual(self.visualInfo.sourceID) then
				enchant = WCollections:GetHiddenEnchant();
			end
			if enchant then
	...
	elseif ( button == "RightButton" ) then
		...
		if ( --[[not self.visualInfo.isCollected or]] self.visualInfo.isHideVisual or transmogType == LE_TRANSMOG_TYPE_ILLUSION ) then
			if not WCollections.Developer then return; end
		end
...
function WardrobeItemsModelMixin:OnEnter()
	...
	if ( self:GetParent().transmogType == LE_TRANSMOG_TYPE_ILLUSION ) then
		...
		local enchant = WCollections:GetEnchantFromScroll(self.visualInfo.sourceID);
		if enchant then
			local variants = WCollections:GetScrollVariantsFromEnchant(enchant);
			if variants then
				for _, variant in ipairs(variants) do
					if variant ~= self.visualInfo.sourceID then
						local _, name = C_TransmogCollection.GetIllusionSourceInfo(variant);
						GameTooltip:AddLine(format(WCollections.L["Tooltip.Transmog.EnchantVariantFormat"], name), GRAY_FONT_COLOR.r, GRAY_FONT_COLOR.g, GRAY_FONT_COLOR.b);
					end
				end
			end
		end
		...
		if WCollections.Developer then
			GameTooltip:AddLine("ID "..self.visualInfo.sourceID.."  ||  "..(WCollections:GetEnchantFromScroll(self.visualInfo.sourceID) or WCollections:GetHiddenEnchant()), 0.25, 0.25, 0.25, 1, 1);
		elseif WCollections.Config.Wardrobe.ShowItemID and not C_TransmogCollection.IsAppearanceHiddenVisual(self.visualInfo.sourceID) then
...
function WardrobeSetsTransmogModelMixin:RefreshTooltip()
	...
	if ( waitingOnQuality ) then
		...
		if WCollections.Config.Wardrobe.ShowSetID then
		...
		if next(failedSlots) then
			GameTooltip:AddLine(" ");
			GameTooltip:AddLine(WCollections.L["Transmog.Sets.MissingSlots"]);
			for _, slot in ipairs({ 1, 3, 15, 5, 19, 4, 9, 10, 6, 7, 8, 16, 17, 18 }) do
				local slotName = WCollections.TransmogrifiableSlots[slot];
				...
				if slotState[slot] ~= nil and slotName then
					...
					for _, source in ipairs(slotSources) do
						...
						if not hasNotStoreOnlySources and not WCollections:IsStoreOrSubscriptionExclusiveItem(source.sourceID) then
							hasNotStoreOnlySources = true;
						end
					end

					if slotState[slot] then
						GameTooltip:AddDoubleLine(slotName, WCollections.L["Transmog.Sets.MissingSlots.Collected"], 1, 1, 1, 0, 1, 0);
					elseif hasCollectedSources then
						GameTooltip:AddDoubleLine(slotName, WCollections.L["Transmog.Sets.MissingSlots.Incompatible"], 1, 1, 1, 1, 0, 0);
					elseif hasNotStoreOnlySources then
						GameTooltip:AddDoubleLine(slotName, WCollections.L["Transmog.Sets.MissingSlots.NotCollected"], 1, 1, 1, 0.5, 0.5, 0.5);
					else
						GameTooltip:AddDoubleLine(slotName, WCollections.L["Transmog.Sets.MissingSlots.NotCollectedStoreOnly"], 1, 1, 1, 0.5, 0.5, 0.5);
					end
				end
			end
		end
...
function WardrobeCollectionFrame_SetAppearanceTooltip(contentFrame, sources, primarySourceID, showSets)
	...
	self:EnableKeyboard(self.tooltipCycle and WCollections.Config.Wardrobe.TooltipCycleKeyboard);
	if contentFrame.tooltipItemFrame then
		contentFrame.tooltipItemFrame:EnableMouseWheel(WCollections.Config.Wardrobe.TooltipCycleMouseWheel);
	end
...
function WardrobeCollectionFrameRightClickDropDown_Init(self, level, menuList)
	...
	local skin = WCollections:GetSkinInfo(appearanceID);
	...
	if level ~= 1 then
		if menuList == "Sets" then
			...
			if skin and skin.Sets then
				for _, setID in ipairs(skin.Sets) do
					local set = WCollections.Cache.Sets[setID];
					info.text = format(WCollections.L["Transmog.Items.ViewSet.Format"], set.name, set.description);
					info.func = function()
						if not CollectionsJournal:IsVisible() or not WardrobeCollectionFrame:IsVisible() then
			...
			for i = 1, C_TransmogCollection.GetNumTransmogSources() do
				if i ~= TRANSMOG_SOURCE_SUBSCRIPTION then
					...
					info.func = function(button, _, _, checked)
						...
						WCollections:SendAddonMessage(format("DEV:SETSOURCEMASK:%d:%d", appearanceID, value));
		...
		elseif menuList == "Subscription" then
			...
			local appearanceSubscriptionID = WCollections.SubscriptionBySkin[appearanceID] or 0;
			for _, name in ipairs(sortedNames) do
				...
				info.func = function(button)
					...
					if appearanceSubscriptionID ~= 0 then
						tDeleteItem(WCollections.Subscriptions[appearanceSubscriptionID].Skins, appearanceID);
						WCollections.SubscriptionBySkin[appearanceID] = nil;
					end
					if skin then
					...
					if subscription ~= 0 then
						WCollections.SubscriptionBySkin[appearanceID] = subscription;
						table.insert(WCollections.Subscriptions[subscription].Skins, appearanceID);
						if skin then
							skin.SourceMask = bit.bor(skin.SourceMask or 0, bit.lshift(1, TRANSMOG_SOURCE_SUBSCRIPTION - 1));
						end
					end
					WCollections:SendAddonMessage(format("DEV:SETSUBSCRIPTION:%d:%d", appearanceID, subscription));
					WCollections:RaiseEvent("TRANSMOG_COLLECTION_UPDATED");
					for i = 1, UIDROPDOWNMENU_MAXBUTTONS do
						local b = _G["DropDownList1Button"..i];
						if b and b:IsShown() and b.menuList == "Subscription" then
							b:SetText(format("Subscription: |cFF00FF00%s|r", subscription ~= 0 and WCollections.Subscriptions[subscription].Name or "- NONE -"));
						end
						b = _G["DropDownList2Button"..i];
						if b and b:IsShown() and b.value then
							local checkImage = _G["DropDownList2Button"..i.."Check"];
		...
		elseif menuList == "Camera" then
			...
			for id, camera in pairs(WCollections.Cache.Cameras) do
			...
			for _, name in ipairs(sortedNames) do
				...
				info.keepShownOnClick = true;
				info.func = function(button)
					local camera = button.value % WCollections.SexToCameraID[1];
					if skin then
						skin.Camera = camera ~= 0 and camera or nil;
					end
					WCollections:SendAddonMessage(format("DEV:SETCAMERA:%d:%d", appearanceID, camera));
					WCollections:RaiseEvent("TRANSMOG_COLLECTION_UPDATED");
					for i = 1, UIDROPDOWNMENU_MAXBUTTONS do
						local b = _G["DropDownList1Button"..i];
						if b and b:IsShown() and b.menuList == "Camera" then
							b:SetText(format("Camera: |cFF00FF00%s|r", camera ~= 0 and select(6, unpack(WCollections.Cache.Cameras[button.value])) or "- NONE -"));
						end
						b = _G["DropDownList2Button"..i];
						if b and b:IsShown() and b.value then
							local checkImage = _G["DropDownList2Button"..i.."Check"];
	...
	if skin and skin.Sets then
		...
		if #skin.Sets == 1 then
			...
			local set = WCollections.Cache.Sets[setID];
			info.text = format(WCollections.L["Transmog.Items.ViewSet.Singular"], set.name, set.description);
			info.func = function()
				if not CollectionsJournal:IsVisible() or not WardrobeCollectionFrame:IsVisible() then
					ToggleCollectionsJournal(COLLECTIONS_JOURNAL_TAB_INDEX_APPEARANCES);
				end
			...
			info.text = WCollections.L["Transmog.Items.ViewSet.Plural"];
			info.hasArrow = true;
			info.menuList = "Sets";
			info.keepShownOnClick = true;
	...
	for i = 1, #sources do
		if ( sources[i].isCollected and not sources[i].useError ) then
			...
			if ( sources[i].name ) then
				...
				WCollections:QueryItem(sources[i].sourceID);
	...
	if WCollections.Developer then
		...
		info = UIDropDownMenu_CreateInfo();
		info.text = format("|T%s:64|t %s", WCollections:GetSkinIcon(appearanceID), select(2, GetItemInfo(appearanceID)));
		info.isTitle = true;
		info.notCheckable = true;
		UIDropDownMenu_AddButton(info);
		...
		info = UIDropDownMenu_CreateInfo();
		info.text = WCollections:HasSkin(appearanceID) and "Lock" or "Unlock";
		info.notCheckable = true;
		info.func = function() WCollections:SendAddonMessage(format("DEV:%sLOCKSKIN:%d", WCollections:HasSkin(appearanceID) and "" or "UN", appearanceID)); end;
		UIDropDownMenu_AddButton(info);
		...
		if WardrobeCollectionFrame.ItemsCollectionFrame.transmogType == LE_TRANSMOG_TYPE_APPEARANCE and not C_TransmogCollection.IsAppearanceHiddenVisual(appearanceID) then
			...
			local camera = cameraID < 0 and WCollections.Cache.Cameras[-cameraID];
			local subscription = WCollections:GetSubscriptionForSkin(appearanceID);
			...
			info.func = function(_, _, _, checked)
				...
				WCollections:SendAddonMessage(format("DEV:SETOBTAINABLE:%d:%d", appearanceID, checked and 0 or 1));
			end;
			UIDropDownMenu_AddButton(info);
...
function WardrobeCollectionFrameSearchBox_OnTextChanged(self)
	...
	elseif not WCollections:UseServersideTextSearch() then
...
function WardrobeFilterDropDown_InitializeItems(self, level)
	...
	if level == 1 then
		...
		info.text = WCollections.L["Transmog.Sets.Filter.ArmorType"]
		info.value = 2;
		UIDropDownMenu_AddButton(info, level);

		info.text = WCollections.L["Transmog.Sets.Filter.Classes"]
		info.value = 3;
		UIDropDownMenu_AddButton(info, level);

		info.text = WCollections.L["Transmog.Sets.Filter.Races"]
		info.value = 4;
		UIDropDownMenu_AddButton(info, level);
		...
		if atTransmogrifier then
			...
			if next(WCollections.UnclaimedQuests) then
				info.text = WCollections.L["Transmog.Items.Filter.Claimable"];
				info.func = function(_, _, _, value)
					WCollections:SetCVarBool("transmogrifyShowClaimable", value);
					WCollections:RaiseEvent("TRANSMOG_COLLECTION_UPDATED");
				end;
				info.checked = function() return WCollections:GetCVarBool("transmogrifyShowClaimable"); end;
				UIDropDownMenu_AddButton(info, level);
			end

			info.text = WCollections.L["Transmog.Items.Filter.Purchasable"];
			info.func = function(_, _, _, value)
				WCollections:SetCVarBool("transmogrifyShowPurchasable", value);
				WCollections:RaiseEvent("TRANSMOG_COLLECTION_UPDATED");
			end;
			info.checked = function() return WCollections:GetCVarBool("transmogrifyShowPurchasable"); end;
			UIDropDownMenu_AddButton(info, level);
		end
	else
		if level == 2 and UIDROPDOWNMENU_MENU_VALUE == 2 then
			...
			local items =
			{
				{ WCollections.L["Transmog.Sets.Filter.ArmorType.Cloth"], 2 },
				{ WCollections.L["Transmog.Sets.Filter.ArmorType.Leather"], 3 },
				{ WCollections.L["Transmog.Sets.Filter.ArmorType.Mail"], 4 },
				{ WCollections.L["Transmog.Sets.Filter.ArmorType.Plate"], 5 },
				{ WCollections.L["Transmog.Sets.Filter.ArmorType.Misc"], 1 },
				{ WCollections.L["Transmog.Sets.Filter.ArmorType.Any"], 6 },
			};
		...
		elseif level == 2 and UIDROPDOWNMENU_MENU_VALUE == 3 then
			...
			for _, class in ipairs(CLASS_SORT_ORDER) do
				...
				if color then
					...
					info.text = format("|cFF%s%s|r", WCollections.RGBPercToHex(color.r, color.g, color.b), LOCALIZED_CLASS_NAMES_MALE[class]);
			...
			local id = WCollections.ClassNameToID["ANY"];
			info.text = WCollections.L["Transmog.Sets.Filter.Classes.Any"];
			info.func = function(_, _, _, value) C_TransmogCollection.SetClassFilter(id, value); end
			info.checked = function() return C_TransmogCollection.IsClassFilterChecked(id) end;
			UIDropDownMenu_AddButton(info, level);
		elseif level == 2 and UIDROPDOWNMENU_MENU_VALUE == 4 then
			...
			for _, race in ipairs(WCollections.RaceSortOrder) do
				local faction = WCollections.RaceNameToFaction[race];
				if lastFaction ~= faction then
					lastFaction = faction;
					UIDropDownMenu_AddSpace(level);
					info.text = format(WCollections.L[faction == FACTION_ALLIANCE and "Transmog.Sets.Filter.Races.Alliance" or "Transmog.Sets.Filter.Races.Horde"], faction);
				...
				local id = WCollections.RaceNameToID[race];
				info.text = WCollections.L["Race."..id];
			...
			UIDropDownMenu_AddSpace(level);
			info.text = WCollections.L["Transmog.Sets.Filter.Races.Neutral"];
		...
		elseif level == 2 and UIDROPDOWNMENU_MENU_VALUE == 1 then
			...
			if WCollections.Developer then
				...
				info.func = function(_, _, _, value)
					WCollections:SetCVarBool("transmogrifyShowObtainable", value);
					WCollections:RaiseEvent("TRANSMOG_COLLECTION_UPDATED");
				end;
				info.checked = function() return WCollections:GetCVarBool("transmogrifyShowObtainable"); end;
				UIDropDownMenu_AddButton(info, level);
				...
				info.tooltipOnButton = 1;
				info.func = function(_, _, _, value)
					WCollections:SetCVarBool("transmogrifyShowUnobtainable", value);
					WCollections:RaiseEvent("TRANSMOG_COLLECTION_UPDATED");
				end;
				info.checked = function() return WCollections:GetCVarBool("transmogrifyShowUnobtainable"); end;
				UIDropDownMenu_AddButton(info, level);
			end
		end
	end
...
function WardrobeFilterDropDown_InitializeBaseSets(self, level)
	...
	local items =
	{
		...
		{ },
		{ WCollections.L["Transmog.Sets.Filter.Core"], LE_TRANSMOG_SET_FILTER_CORE },
		{ WCollections.L["Transmog.Sets.Filter.Wowhead"], LE_TRANSMOG_SET_FILTER_WOWHEAD },
		{ },
		{ WCollections.L["Transmog.Sets.Filter.ArmorType"], 1,
		{
			...
			{ WCollections.L["Transmog.Sets.Filter.ArmorType.Cloth"], LE_TRANSMOG_SET_FILTER_CLOTH },
			{ WCollections.L["Transmog.Sets.Filter.ArmorType.Leather"], LE_TRANSMOG_SET_FILTER_LEATHER },
			{ WCollections.L["Transmog.Sets.Filter.ArmorType.Mail"], LE_TRANSMOG_SET_FILTER_MAIL },
			{ WCollections.L["Transmog.Sets.Filter.ArmorType.Plate"], LE_TRANSMOG_SET_FILTER_PLATE },
			{ WCollections.L["Transmog.Sets.Filter.ArmorType.Misc"], LE_TRANSMOG_SET_FILTER_MISC },
		} },
		{ WCollections.L["Transmog.Sets.Filter.Classes"], 2, function(value, level)
			local info = { };
			info.keepShownOnClick = true;
			info.isNotRadio = true;
			info.hasArrow = false;
		...
		{ WCollections.L["Transmog.Sets.Filter.Races"], 4, function(value, level)
			local info = { };
			info.keepShownOnClick = true;
			info.isNotRadio = true;
			info.hasArrow = false;
		...
		{ SOURCES, 3,
		{
			...
			{ WCollections.L["Transmog.Sets.Filter.Store"], LE_TRANSMOG_SET_FILTER_STORE },
		} },
		...
		{ },
		{ WCollections.L["Transmog.Sets.Filter.Group"], LE_TRANSMOG_SET_FILTER_GROUP },
		{ WCollections.L["Transmog.Sets.Filter.Sort"], LE_TRANSMOG_SET_FILTER_SORT, true },
	};
...
function WardrobeFilterDropDown_InitializeUsableSets(self, level)
	...
	if not self.setsValidSlotNames and next(WCollections.Cache.Sets) then
		...
		for _, set in pairs(WCollections.Cache.Sets) do
			for _, source in ipairs(set.sources) do
				...
				local slotName = slot and WCollections.TransmogrifiableSlots[slot];
				if slotName then
					self.setsValidSlotNames[slot] = slotName;
				end
			end
		end
		self.setsValidSlotNames[EQUIPPED_LAST + 1] = WCollections.L["Transmog.Sets.Filter.Slots.Store"];
	end
	local validSlotNames = self.setsValidSlotNames;

	local info = UIDropDownMenu_CreateInfo();
	info.text = WCollections.L["Transmog.Sets.Filter.Slots.RequiredSlots"];
	info.isTitle = true;
	info.notCheckable = true;
	UIDropDownMenu_AddButton(info, level);
	...
	for _, i in ipairs({ 1, 3, 15, 5, 19, 4, 9, 10, 6, 7, 8, 16, 17, 18, EQUIPPED_LAST + 1 }) do
		...
		if slot then
			...
			if i == EQUIPPED_LAST + 1 then
				info.tooltipTitle = WCollections.L["Transmog.Sets.Filter.Slots.Store.Header"];
				info.tooltipText = WCollections.L["Transmog.Sets.Filter.Slots.Store.Text"];
			else
				info.tooltipTitle = format(WCollections.L["Transmog.Sets.Filter.Slots.RequiredSlots.Header"], info.text);
				info.tooltipText = format(WCollections.L["Transmog.Sets.Filter.Slots.RequiredSlots.Text"], info.text);
			end
			UIDropDownMenu_AddButton(info, level);
		end
	end
	...
	info.func = function()
		...
		if numDisabled > WCollections.SearchMaxSetsSlotMask then
			StaticPopup_Show("WCOLLECTIONS_ERROR", format(WCollections.L["Popup.Error.MaxSetsSlotMask"], WCollections.SearchMaxSetsSlotMask));
			return;
		end
		WCollections:SetCVar("transmogrifySetsSlotMask", self.setsSlotMask);
		WCollections:RaiseEvent("TRANSMOG_COLLECTION_UPDATED");
	end;
	UIDropDownMenu_AddButton(info, level);
...
function WardrobeTransmogFrameSpecDropDown_Initialize()
	...
	local currentSpecOnly = WCollections:GetCVarBool("transmogCurrentSpecOnly");
	...
	info.checked = false;
	info.text = format([[|TInterface\AddOns\WCollections\Interface\Common\spacer:1:20:-1:-1|t |T%s:22:22:0:-1|t %s (%s)]], icon, name, GetSpecialization() == 1 and TALENT_SPEC_PRIMARY or TALENT_SPEC_SECONDARY);
	UIDropDownMenu_AddButton(info, UIDROPDOWNMENU_MENU_LEVEL);
end

function WardrobeTransmogFrameSpecDropDown_OnClick(self)
	WCollections:SetCVarBool("transmogCurrentSpecOnly", self.value == 1);
end
...
function WardrobeSetsDataProviderMixin:SortSets(sets, reverseUIOrder, variantSort)
	...
	local comparison = variantSort and function(a, b)
		local aOrder = a.description and WCollections.ItemNameDescriptions[a.description];
		local bOrder = b.description and WCollections.ItemNameDescriptions[b.description];
...
function WardrobeSetsDataProviderMixin:GetIconForSet(setID)
	...
	if ( not sourceData.icon ) then
		...
		if ( sortedSources[1] ) then
			...
			sourceData.icon = WCollections:GetSkinIcon(itemID);
			if not sourceData.icon then
...
function WardrobeSetsCollectionMixin:OnShow()
	--WCollections:RegisterEvent(self, "GET_ITEM_INFO_RECEIVED");
	WCollections:RegisterEvent(self, "TRANSMOG_COLLECTION_ITEM_UPDATE");
	WCollections:RegisterEvent(self, "TRANSMOG_COLLECTION_UPDATED");
	...
	if (self:GetParent().SetsTabHelpBox:IsShown()) then
		self:GetParent().SetsTabHelpBox:Hide()
		WCollections:SetCVarBitfield("closedInfoFrames", LE_FRAME_TUTORIAL_TRANSMOG_SETS_TAB, true);
	end
	-- Fix frame levels
...
function WardrobeSetsCollectionMixin:OnHide()
	WCollections:UnregisterEvent(self, "GET_ITEM_INFO_RECEIVED");
	WCollections:UnregisterEvent(self, "TRANSMOG_COLLECTION_ITEM_UPDATE");
	WCollections:UnregisterEvent(self, "TRANSMOG_COLLECTION_UPDATED");
...
function WardrobeSetsCollectionMixin:DisplaySet(setID)
	...
	self.DetailsFrame.LabelID:SetShown(WCollections.Config.Wardrobe.ShowSetID);
	self.DetailsFrame.Wowhead:SetShown(WCollections.Config.Wardrobe.ShowWowheadSetIcon and bit.band(setInfo.flags, 0x1000) ~= 0);
	...
	for i = 1, #sortedSources do
		...
		local storeSource = WCollections:GetStoreSetSource(setID, slot);
		local subscriptionSource = WCollections:GetSubscriptionForSetSource(setID, slot);
		itemFrame.ClaimQuest:SetShown(not sortedSources[i].collected and WCollections:CanClaimSetSlotSkin(setID, slot));
...
function WardrobeSetsCollectionMixin:OpenVariantSetsDropDown()
	...
	local baseSet = WCollections.Cache.Sets[baseSetID];
	if baseSet and baseSet.Variants then
		local numHidden = (#baseSet.Variants + 1) - #variantSets;
		if numHidden > 0 then
			info.text = format(WCollections.L["Transmog.Sets.VariantsHidden"], numHidden);
			info.disabled = true;
			info.func = nil;
			info.checked = false;
			UIDropDownMenu_AddButton(info);
...
function WardrobeSetsCollectionScrollFrameMixin:OnShow()
	WCollections:RegisterEvent(self, "TRANSMOG_SETS_UPDATE_FAVORITE");
	WCollections:RegisterEvent(self, "GET_ITEM_INFO_RECEIVED");
end

function WardrobeSetsCollectionScrollFrameMixin:OnHide()
	WCollections:UnregisterEvent(self, "TRANSMOG_SETS_UPDATE_FAVORITE");
	WCollections:UnregisterEvent(self, "GET_ITEM_INFO_RECEIVED");
end
...
function WardrobeSetsTransmogMixin:OnShow()
	...
	WCollections:RegisterEvent(self, "TRANSMOG_COLLECTION_ITEM_UPDATE");
	WCollections:RegisterEvent(self, "TRANSMOG_COLLECTION_UPDATED");
	WCollections:RegisterEvent(self, "PLAYER_EQUIPMENT_CHANGED");
	self:RegisterEvent("PLAYER_EQUIPMENT_CHANGED");
	WCollections:RegisterEvent(self, "TRANSMOG_SETS_UPDATE_FAVORITE");
	self:RefreshCameras();
	...
	if (self:GetParent().SetsTabHelpBox:IsShown()) then
		self:GetParent().SetsTabHelpBox:Hide();
		WCollections:SetCVarBitfield("closedInfoFrames", LE_FRAME_TUTORIAL_TRANSMOG_SETS_VENDOR_TAB, true);
	end
end
...
function WardrobeSetsTransmogMixin:OnHide()
	...
	WCollections:UnregisterEvent(self, "TRANSMOG_COLLECTION_ITEM_UPDATE");
	WCollections:UnregisterEvent(self, "TRANSMOG_COLLECTION_UPDATED");
	WCollections:UnregisterEvent(self, "PLAYER_EQUIPMENT_CHANGED");
	self:UnregisterEvent("PLAYER_EQUIPMENT_CHANGED");
	WCollections:UnregisterEvent(self, "TRANSMOG_SETS_UPDATE_FAVORITE");
	...
	for sourceID in pairs(sources) do
		...
		for i, slotSourceInfo in ipairs(slotSources) do
			if ( not slotSourceInfo.name ) then
				waitingOnData = true;
				WCollections:QueryItem(slotSourceInfo.sourceID);
			end
		end
	end
...
function WardrobeSetsTransmogMixin:OpenRightClickDropDown()
	...
	info.text = WCollections.L["Transmog.Sets.ViewInCollections"];
	info.func = function()
		if not CollectionsJournal:IsVisible() or not WardrobeCollectionFrame:IsVisible() then
			ToggleCollectionsJournal(COLLECTIONS_JOURNAL_TAB_INDEX_APPEARANCES);
		end
