...
function ToyBox_OnLoad(self)
	...
	WCollections:UIDropDownMenu_Initialize(self.toyOptionsMenu, ToyBoxOptionsMenu_Init, "MENU");

	WCollections:RegisterEvent(self, "TOYS_UPDATED");
...
function ToyBox_OnShow(self)
	WCollections:SetCVarBitfield("closedInfoFrames", LE_FRAME_TUTORIAL_TOYBOX, true);

	if(C_ToyBox.HasFavorites()) then 
		WCollections:SetCVarBitfield("closedInfoFrames", LE_FRAME_TUTORIAL_TOYBOX_FAVORITE, true);
		self.favoriteHelpBox:Hide();
	end

	SetPortraitToTexture(CollectionsJournalPortrait, [[Interface\AddOns\WCollections\Interface\Icons\Trade_Archaeology_ChestofTinyGlassAnimals]]);
...
function ToyBox_OnMouseWheel(self, value)
	WCollections:SetCVarBitfield("closedInfoFrames", LE_FRAME_TUTORIAL_TOYBOX_MOUSEWHEEL_PAGING, true);
...
function ToyBoxOptionsMenu_Init(self, level)
	...
	if WCollections.Developer then
		local toyID = WCollections:GetToyIDByItem(ToyBox.menuItemID) or 0;
		...
		info = UIDropDownMenu_CreateInfo();
		info.text = WCollections:HasToy(toyID) and "Lock" or "Unlock";
		info.notCheckable = true;
		info.func = function() WCollections:SendAddonMessage(format("DEV:%sLOCKTOY:%d", WCollections:HasToy(toyID) and "" or "UN", toyID)); end;
		UIDropDownMenu_AddButton(info);

		info = UIDropDownMenu_CreateInfo();
		info.text = "Add Item";
		info.notCheckable = true;
		info.func = function() WCollections:SendAddonCommand(format(".additem %d", ToyBox.menuItemID)); end;
		UIDropDownMenu_AddButton(info);

		info = UIDropDownMenu_CreateInfo();
		info.text = "Delete Item";
		info.notCheckable = true;
		info.func = function() WCollections:SendAddonCommand(format(".additem %d -1", ToyBox.menuItemID)); end;
		UIDropDownMenu_AddButton(info);

		info = UIDropDownMenu_CreateInfo();
		info.text = "Clear Cooldown";
		info.notCheckable = true;
		info.func = function() table.wipe(WCollections.ItemCooldowns); SendChatMessage(".cooldown", "SAY"); end;
		UIDropDownMenu_AddButton(info);
	end
end
...
function ToySpellButton_OnHide(self)
	...
	WCollections:UnregisterEvent(self, "TOYS_UPDATED");	
end
...
function ToySpellButton_OnClick(self, button)
	if ( button ~= "LeftButton" ) then
		if (PlayerHasToy(self.itemID)) or WCollections.Developer then
...
function ToySpellButton_UpdateButton(self)
	...
	if (PlayerHasToy(self.itemID)) then
		...
		if (ToyBox.firstCollectedToyID == self.itemID and not ToyBox.favoriteHelpBox:IsVisible() and not WCollections:GetCVarBitfield("closedInfoFrames", LE_FRAME_TUTORIAL_TOYBOX_FAVORITE)) then
			ToyBox.favoriteHelpBox:Show();
			ToyBox.favoriteHelpBox:SetPoint("TOPLEFT", self, "BOTTOMLEFT", -5, -20);
		end

		local toyID = WCollections:GetToyIDByItem(self.itemID) or 0;
		self.SubscriptionOverlay:SetShown(not WCollections:HasToy(toyID) and WCollections:IsActiveToySubscriptionToy(toyID));
	else
		iconTexture:Hide();
	...
	self.active:SetShown(WCollections.ActiveToys[self.itemID]);
end
...
function ToyBox_UpdateButtons()
	...
	ToyBox.SubscriptionStatus.SubscriptionInfo:SetShown(WCollections:IsActiveToySubscription());
...
function ToyBox_CollectAvailableFilters()
	...
	for _, itemID in ipairs(C_ToyBox.GetToys()) do
		local sourceType = select(4, WCollections:GetToyInfoByItem(itemID));
		ToyBox.baseFilterTypes[sourceType and sourceType + 1 or 12] = true;
	end
end

function ToyBoxFilterDropDown_OnLoad(self)
	WCollections:UIDropDownMenu_Initialize(self, ToyBoxFilterDropDown_Initialize, "MENU");
end
...
function ToyBoxFilterDropDown_Initialize(self, level)
	...
	if level == 1 then
		...
		if WCollections:IsActiveToySubscription() then
			info.text = WCollections.L["Toy.Filter.Subscription"];
			info.func = function(_, _, _, value)
