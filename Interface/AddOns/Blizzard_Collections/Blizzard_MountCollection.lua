...
function MountJournal_OnLoad(self)
	...
	WCollections:RegisterEvent(self, "MOUNT_JOURNAL_USABILITY_CHANGED");
	WCollections:RegisterEvent(self, "MOUNT_JOURNAL_SEARCH_UPDATED");
	...
	WCollections:UIDropDownMenu_Initialize(self.mountOptionsMenu, MountOptionsMenu_Init, "MENU");
end
...
function MountJournal_UpdateMountList()
	...
	for i=1, #buttons do
		...
		if ( displayIndex <= numDisplayedMounts and showMounts ) then
			...
			button.SubscriptionOverlay:SetShown(WCollections:IsActiveMountSubscriptionMount(mountID));
	...
	MountJournal.SubscriptionStatus.PremiumInfo:SetShown(WCollections:IsActiveMountPremium());
	MountJournal.SubscriptionStatus.SubscriptionInfo:SetShown(WCollections:IsActiveMountSubscription());
...
function MountJournal_UpdateMountDisplay()
	if ( MountJournal.selectedMountID ) then
		...
		if creatureName and ( MountJournal.MountDisplay.lastDisplayed ~= spellID ) then
			...
			MountJournal.MountDisplay.InfoButton.SubscriptionInfo:SetText(WCollections:IsActiveMountSubscriptionMount(MountJournal.selectedMountID) and WCollections.L["Mount.Subscription.Details.Info"] or "");
...
function MountListDragButton_OnDoubleClick(self, button)
	ClearCursor();
	local parent = self:GetParent();
	if button == "LeftButton" and WCollections.Config.Wardrobe.MountsDoubleClickIcon then
...
function MountListItem_OnDoubleClick(self, button)
	if button == "LeftButton" and WCollections.Config.Wardrobe.MountsDoubleClickName then
...
function MountJournalFilterDropDown_OnLoad(self)
	WCollections:UIDropDownMenu_Initialize(self, MountJournalFilterDropDown_Initialize, "MENU");
end
...
function MountJournalFilterDropDown_Initialize(self, level)
	...
	if level == 1 then
		...
		if WCollections:IsActiveMountSubscription() then
			info.text = WCollections.L["Mount.Filter.Subscription"];
			info.func = function(_, _, _, value)
...
function MountJournalSummonRandomFavoriteButton_OnEnter(self)
	...
	--GameTooltip:SetHyperlink("spell:"..self.spellID);
	GameTooltip:SetText(WCollections.L["Macro.Mount.Name"], HIGHLIGHT_FONT_COLOR.r, HIGHLIGHT_FONT_COLOR.g, HIGHLIGHT_FONT_COLOR.b);
	GameTooltip:AddLine(WCollections.L[C_MountJournal.GetFavoriteMacro() and "Macro.Mount.Desc.Created" or "Macro.Mount.Desc"], NORMAL_FONT_COLOR.r, NORMAL_FONT_COLOR.g, NORMAL_FONT_COLOR.b, 1);
	GameTooltip:Show();
end
