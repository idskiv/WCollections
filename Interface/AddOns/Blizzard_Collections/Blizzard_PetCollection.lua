...
function GetPetTypeTexture(petType)
	if PET_TYPE_SUFFIX[petType] then
		return [[Interface\AddOns\WCollections\Interface\PetBattles\PetIcon-]]..PET_TYPE_SUFFIX[petType];
	else
		return [[Interface\AddOns\WCollections\Interface\PetBattles\PetIcon-NO_TYPE]];
	end
end
...
function PetJournal_OnLoad(self)
	...
	WCollections:RegisterEvent(self, "PET_JOURNAL_SEARCH_UPDATED");
	...
	WCollections:UIDropDownMenu_Initialize(self.petOptionsMenu, PetOptionsMenu_Init, "MENU");
end
...
function PetJournal_UpdatePetList()
	...
	for i=1, #buttons do
		...
		if ( displayIndex <= numDisplayedPets and showPets ) then
			...
			button.SubscriptionOverlay:SetShown(WCollections:IsActivePetSubscriptionPet(petID));
			...
			button.name:SetText("");
			button.icon:SetTexture([[Interface\AddOns\WCollections\Interface\PetBattles\MountJournalEmptyIcon]]);
			button.new:Hide();
			button.newGlow:Hide();
	...
	PetJournal.SubscriptionStatus.SubscriptionInfo:SetShown(WCollections:IsActivePetSubscription());
...
function PetJournal_UpdatePetDisplay()
	if ( PetJournal.selectedPetID ) then
		...
		if creatureName and ( PetJournal.PetDisplay.lastDisplayed ~= spellID ) then
			...
			PetJournal.PetDisplay.InfoButton.SubscriptionInfo:SetText(WCollections:IsActivePetSubscriptionPet(PetJournal.selectedPetID) and WCollections.L["Pet.Subscription.Details.Info"] or "");
		...
		PetJournal.TypeInfo.typeIcon:SetTexture([[Interface\AddOns\WCollections\Interface\PetBattles\PetIcon-]]..PET_TYPE_SUFFIX[petType]);
...
function PetJournal_CollectAvailableFilters()
	...
	for _, petID in ipairs(C_PetJournal.GetPetIDs()) do
		local sourceType = select(6, WCollections:GetPetInfo(petID));
...
function PetListDragButton_OnDoubleClick(self, button)
	ClearCursor();
	local parent = self:GetParent();
	if button == "LeftButton" and WCollections.Config.Wardrobe.PetsDoubleClickIcon then
...
function PetListItem_OnDoubleClick(self, button)
	if button == "LeftButton" and WCollections.Config.Wardrobe.PetsDoubleClickName then
...
function PetJournalFilterDropDown_OnLoad(self)
	WCollections:UIDropDownMenu_Initialize(self, PetJournalFilterDropDown_Initialize, "MENU");
end
...
function PetJournalFilterDropDown_Initialize(self, level)
	...
	if level == 1 then
		...
		if WCollections:IsActivePetSubscription() then
			info.text = WCollections.L["Pet.Filter.Subscription"];
			info.func = function(_, _, _, value)
...
function PetJournalSummonRandomFavoritePetButton_OnEnter(self)
	...
	--GameTooltip:SetHyperlink("spell:"..self.spellID);
	GameTooltip:SetText(WCollections.L["Macro.Pet.Name"], HIGHLIGHT_FONT_COLOR.r, HIGHLIGHT_FONT_COLOR.g, HIGHLIGHT_FONT_COLOR.b);
	GameTooltip:AddLine(WCollections.L[C_PetJournal.GetFavoriteMacro() and "Macro.Pet.Desc.Created" or "Macro.Pet.Desc"], NORMAL_FONT_COLOR.r, NORMAL_FONT_COLOR.g, NORMAL_FONT_COLOR.b, 1);
	GameTooltip:Show();
end
