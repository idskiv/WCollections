-- +OORRGIISS
-- -OORRGCCCC
WCollections.CameraOptions =
{
    "Classic",
    "HD2017",
    "HD2019",
};
WCollections.CameraOptionsToCameraID = { };
for id, name in ipairs(WCollections.CameraOptions) do
    WCollections.CameraOptionsToCameraID[name] = id * 10000000;
end
WCollections.RaceToCameraID =
{
    Human                  =  1 *   100000,
    Orc                    =  2 *   100000,
    Dwarf                  =  3 *   100000,
...
WCollections.SexToCameraID =
{
    [1]                    =  1 *    10000,
    [2]                    =  1 *    10000,
    [3]                    =  2 *    10000,
};
WCollections.InvTypeToCameraID =
{
...
WCollections.SubClassToCameraID =       1;

function WCollections:GetCharacterCameraID(invTypeName, cameraID)
    local fallback = self.CameraOptionsToCameraID[WCollections.CameraOptions[1]];
    local option = self.CameraOptionsToCameraID[WCollections.Config.Wardrobe.CameraOption];
    ...
    if cameraID then
        ...
        if WCollections.Cache.Cameras[fullCameraID] then
            return -fullCameraID;
        elseif WCollections.Cache.Cameras[fallbackCameraID] then
...
function WCollections:GetWeaponCameraID(invTypeName, subType, cameraID)
    if cameraID then
        if WCollections.Cache.Cameras[cameraID] then
            return -cameraID;
        end
    end
...
function WCollections:PlayerHasDifferentCameraOptions()
    local race = self.RaceToCameraID[select(2, UnitRace("player"))];
    local sex = self.SexToCameraID[UnitSex("player")];
    local fallbackOption;
    for _, name in ipairs(WCollections.CameraOptions) do
        ...
        if not fallbackOption then
            ...
            for _, invType in ipairs(cameraOptionAffectedInvTypes) do
                local fallback = WCollections.Cameras[fallbackOption + race + sex + invType];
                local camera = WCollections.Cameras[option + race + sex + invType];
...
-- Populate WCollections.Cameras
local O = WCollections.CameraOptionsToCameraID;
local R = WCollections.RaceToCameraID;
local I = WCollections.InvTypeToCameraID;
local S = { Male = WCollections.SexToCameraID[2], Female = WCollections.SexToCameraID[3] }
local BACK = PI;
local MHAND = PI / 2;
local OHAND = 0;
local function Wpn(invType, subType)
    return invType + (subType and (subType + 1) or 0) * WCollections.SubClassToCameraID;
end
